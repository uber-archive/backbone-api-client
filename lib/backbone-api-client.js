...
exports.sync = function (method, model, options) {
  ...
  var reqParams = _.extend(params, options);
  ...
  if (this.beforeCallApiClient) {
    reqParams = this.beforeCallApiClient(method, reqParams) || reqParams;
  }

  // Invoke our apiClient methods (e.g. `_create/_read`)
  var request = options.request = this.callApiClient(method, reqParams, function handleResult (err, resp) {
    if (err) {
